...
const PreStreamingAnimation: React.FC<PreStreamingAnimationProps> = ({
  ...
  return (
    <div className="fixed inset-0 w-screen h-screen flex items-center justify-center bg-ink z-50">
      <div className="relative w-full h-full">
        {/* Cinema Screen Background - Full viewport */}
        <div className="absolute inset-0 overflow-hidden bg-surface border-x-8 border-t-8 border-hairline rounded-t-xl">
          {/* Close Button */}
          <button
            onClick={onBack}
            className="absolute top-4 right-4 z-50 bg-ink/60 text-text p-2 rounded-full border border-hairline hover:bg-ink/80 transition-colors"
            aria-label="Close"
          >
          ...
          <div className="relative h-full flex flex-col items-center justify-center z-10 p-8">
            ...
            <h1 className="font-display text-3xl md:text-4xl font-light text-text text-center mb-8 tracking-tight animate-fade-in">
              {movieTitle}
            </h1>
            ...
            <div className="w-full max-w-lg h-64 relative mx-auto">
              ...
              >
                ...
                <h2 className="text-2xl font-semibold text-text mt-6 mb-2 transition-all duration-500">
                  Getting Your Snacks Ready
                </h2>
                <p
                  id="loading-message"
                  className="text-muted transition-all duration-500 ease-in-out"
                >
                  {message}
                </p>
              </div>
              ...
              >
                ...
                <p
                  id="movie-fact"
                  className="text-muted text-sm transition-all duration-500 ease-in-out"
                >
                  {fact}
                </p>
              </div>
              ...
              >
                <div className="relative h-32 w-32 flex items-center justify-center transition-transform duration-1000 transform hover:scale-110">
                  ...
                  <div
                    className="absolute inset-0 border-4 border-gold rounded-full opacity-80 transition-all duration-300"
                    style={{
                      clipPath: `polygon(50% 50%, 50% 0%, ${50 + 50 * Math.sin((countdown/5) * Math.PI * 2)}% ${50 - 50 * Math.cos((countdown/5) * Math.PI * 2)}%, 50% 50%)`
                    }}
                  ></div>
                  <span className="text-6xl font-bold text-gold transition-all duration-300 transform scale-100">{countdown}</span>
                </div>
                <h2 className="text-2xl font-semibold text-text mt-6 mb-2 transition-all duration-500">
                  Starting Soon!
                </h2>
                <p id="loading-message" className="text-muted transition-all duration-500 ease-in-out">
            ...
            <div className="w-full max-w-md mt-12 relative">
              <div className="flex justify-between text-sm text-muted mb-1">
                ...
                <span className="text-gold transition-all duration-300 ease-in-out">{Math.round(progress)}%</span>
              </div>

              <div className="relative h-6 bg-ink rounded-md overflow-hidden border border-hairline">
                {/* Film Strip Decoration */}
                <div className="absolute inset-y-0 left-0 right-0 flex justify-between items-center">
                  {Array.from({ length: 20 }).map((_, i) => (
                    <div key={i} className="h-full w-1.5 bg-surface"></div>
                  ))}
                </div>
              ...
              <div className="flex flex-wrap justify-between mt-3 text-xs text-muted">
                <div className="transition-all duration-500 ease-in-out hover:text-text">
                  <span className="block text-muted/60">Download Speed</span>
                  <span className="text-text">{downloadSpeed.toFixed(2)} KB/s</span>
                </div>
                <div className="transition-all duration-500 ease-in-out hover:text-text">
                  <span className="block text-muted/60">Peers</span>
                  <span className="text-text">{numPeers}</span>
                </div>
                <div className="transition-all duration-500 ease-in-out hover:text-text">
                  <span className="block text-muted/60">Estimated Wait</span>
                  <span className="text-text">{formatTimeRemaining()}</span>
                </div>
              </div>
            </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 h-20 bg-surface-2 rounded-b-xl overflow-hidden flex border-t border-hairline">
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className="flex-1 border-r border-hairline pt-2">
              <div className="h-4 bg-surface rounded-t-md mx-1"></div>
              <div className="h-8 bg-surface rounded-b-md mx-0.5 mt-1"></div>
            </div>
          ))}
        </div>
      </div>
