...
// ---------------------------------------------------------------------------
// Mock BrowseScreen to keep tests focused on HomeBrowse fetch/compose logic
// ---------------------------------------------------------------------------
...
describe('HomeBrowse', () => {
  it('shows the skeleton initially (synchronous render with pending data)', async () => {
    ...
    (moviesService.browse as ReturnType<typeof vi.fn>).mockImplementation(
      async (params: { api?: string; sort?: string; genre?: number }) => {
        await gate;
        // After the gate opens, return correct data
  ...
  it('renders the "Trending Now" row title', async () => {
    render(<HomeBrowse />);
  ...
  it('renders the "Top 10 Movies This Week" ranked row title', async () => {
    render(<HomeBrowse />);
    const rowTitles = await screen.findAllByTestId('mock-row-title');
    const titles = rowTitles.map((el) => el.textContent);
    expect(titles).toContain('Top 10 Movies This Week');
  });
  ...
  it('calls moviesService.browse 5 times (popular, latest, top_rated, action, drama)', async () => {
    render(<HomeBrowse />);
    await screen.findByTestId('mock-browse-screen');
